...
function parseFrenchDate(input: string): Date {
  ...
  const m = input.trim().toLowerCase().match(/(\d{1,2})\s+([a-zéèêûôîàùç]+)\s+(\d{4})/i);
...
function parseCityAndCountry(location: string): { address: string; city: string; country: string } {
...
async function main() {
  ...
  for (const e of allEvents) {
    const cat = categoryByName.get(e.category) ?? Array.from(categoryByName.values())[0];
    if (!cat) {
    ...
    const existing = await prisma.event.findFirst({ where: { title: e.title, date } });
    if (existing) {
      continue;
    }
