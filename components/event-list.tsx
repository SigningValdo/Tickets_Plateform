...
import { Event, EventCategory } from "@/lib/generated/prisma"
...
export function EventList() {
  ...
  const {data, isPending, error} = useQuery({
    ...
    queryFn: async (): Promise<EventAndCategory[]> => fetch(`/api/admin/events?search=${search}&category=${category}&date=${date}&location=${location}&dateFilter=${dateFilter}`).then(res => res.json()),
  })

  if (isPending) {
    return (
      <div className="text-center py-12">
        <h3 className="text-xl font-medium mb-2">Chargement des événements...</h3>
      </div>
    )
  }
            ...
            <CardContent className="flex-1 p-6">
              <div className="flex flex-col h-full justify-between">
                <div>
                  ...
                  <div className="flex flex-wrap gap-4 mb-4">
                    <div className="flex items-center text-gray-500">
                      ...
                      <span className="text-sm">{event.date && moment(event.date).isValid() ? moment(event.date).format("DD MMM YYYY") : "-"}</span>
                    </div>
                    <div className="flex items-center text-gray-500">
                ...
                <div className="flex items-center justify-between mt-4">
                  <span className="font-bold text-purple-600 text-lg">{event.status}</span>
